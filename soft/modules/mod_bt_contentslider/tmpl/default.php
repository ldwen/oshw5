...
<div id="btcontentslider<?php echo $module->id; ?>" style="display:none;width:<?php echo $moduleWidthWrapper;?>" class="bt-cs<?php echo $moduleclass_sfx? ' bt-cs'.$params->get('moduleclass_sfx'):'';?>">
	...
	<?php foreach( $pages as $key => $list ): ?>
		...
		<?php foreach( $list as $i => $row ): ?>
			...
			<div class="bt-row <?php if($i==0) echo 'bt-row-first'; else if($i==count($list)-1) echo 'bt-row-last' ?>" id="right"  style="width:<?php echo $itemWidth;?>%" >

			<?php if($num >= $sum_rowcol) { ?>
			<div class="media-left media-middle?>" > <i class="<?php echo $arrayName[$i]?>"></i> </div>
			<?php }?>
